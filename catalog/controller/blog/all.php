		...
		$description = $descriptions[$this->config->get('config_language_id')];
			...
			foreach ($results as $result) {
				...
				$this->data['articles'][] = array(
					...
					'image' => $image,
					'image2' => $image2,
					'href'  => $this->url->link('blog/article', '&news_id=' . $result['news_id']),
					'href_comment'  => $this->url->link('blog/article', '&news_id=' . $result['news_id'] . '#comment_area'),
			...
			$this->data['sorts'][] = array(
				...
				'href'  => $this->url->link('blog/all', 'sort=order-A-date-D' . $l_url)
			);

			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_oldest_first'),
				'value' => 'order-A-date-A',
				'href'  => $this->url->link('blog/all', 'sort=order-A-date-A' . $l_url)
			);
			
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'title-A',
				'href'  => $this->url->link('blog/all', 'sort=title-A' . $l_url)
			); 
	
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'title-D',
				'href'  => $this->url->link('blog/all', 'sort=title-D' . $l_url)
			);
			...
			foreach ($plimits as $plimit)
			{
				$this->data['limits'][] = array(
					'text'  => $plimit,
					'value' => $plimit,
					'href'  => $this->url->link('blog/all', $s_url . '&limit=' . $plimit)
				);
			}
			...
			$pagination->url = $this->url->link('blog/all', $s_url . $l_url . '&page={page}');
