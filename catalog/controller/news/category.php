...
class ControllerNewsCategory extends Controller {
	public function index() {  
		...
		if ($category_info) {
			...
			$this->document->addLink($this->url->link('news/category', 'npath=' . $this->model_catalog_news->getFullPath($category_id)), 'canonical');			
			...
			foreach ($results as $result) {
				...
				$this->data['categories'][] = array(
					...
					'href'  => $this->url->link('news/category', 'npath=' . $this->request->get['npath'] . '_' . $result['news_category_id'] . $s_url . $l_url),
				);
			}				
			...
			$this->data['sorts'][] = array(
				...
				'href'  => $this->url->link('news/category', 'sort=order-A-date-D' . $p_url . $l_url),
			);

			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_oldest_first'),
				'value' => 'order-A-date-A',
				'href'  => $this->url->link('news/category', 'sort=order-A-date-A' . $p_url . $l_url)
			);
			
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'title-A',
				'href'  => $this->url->link('news/category', 'sort=title-A' . $p_url . $l_url)
			); 
	
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'title-D',
				'href'  => $this->url->link('news/category', 'sort=title-D' . $p_url . $l_url)
			);
			...
			foreach ($plimits as $plimit)
			{
				$this->data['limits'][] = array(
					'text'  => $plimit,
					'value' => $plimit,
					'href'  => $this->url->link('news/category', $p_url . $s_url . '&limit=' . $plimit)
				);
			}
			...
			$pagination->url = $this->url->link('news/category', $p_url . $s_url . $l_url . '&page={page}');
