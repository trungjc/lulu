	...
	public function index() { 
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'	  => $this->url->link('news/search', $f_url),
		...
		$this->document->addLink(str_replace('&amp;', '&', $this->url->link('news/search', $f_url)), 'canonical');
		...
		if (isset($this->request->get['filter_title']) || isset($this->request->get['filter_tag'])) {
			...
			foreach ($results as $result) {
				...
				$this->data['articles'][] = array(
					...
					'image' => $image,
					'image2' => $image2,
					'href'	=> $this->url->link('news/news', $f_url . '&news_id=' . $result['news_id']),
					'href_comment'  => $this->url->link('news/news', $f_url . '&news_id=' . $result['news_id'] . '#comment_area'),
			...
			$this->data['sorts'][] = array(
				...
				'href'  => $this->url->link('news/search', 'sort=order-A-date-D' . $f_url . $l_url)
			);
			
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_oldest_first'),
				'value' => 'order-A-date-A',
				'href'  => $this->url->link('news/search', 'sort=order-A-date-A' . $f_url . $l_url)
			);

			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_asc'),
				'value' => 'title-A',
				'href'  => $this->url->link('news/search', 'sort=title-A' . $f_url . $l_url)
			); 
	
			$this->data['sorts'][] = array(
				'text'  => $this->language->get('text_name_desc'),
				'value' => 'title-D',
				'href'  => $this->url->link('news/search', 'sort=title-D' . $f_url . $l_url)
			);
			...
			foreach ($plimits as $plimit)
			{
				$this->data['limits'][] = array(
					'text'  => $plimit,
					'value' => $plimit,
					'href'  => $this->url->link('news/search', $f_url . $s_url . '&limit=' . $plimit)
				);
			}
			...
			$pagination->url = $this->url->link('news/search', $f_url . $s_url . $l_url . '&page={page}');
